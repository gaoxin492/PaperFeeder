...
ZH_CN_PACK = SummaryLanguagePack(
    ...
    task_requirements=(
        ...
        "论文筛选 section 里，除了主推深读的几篇外，对今天论文池里剩下没有展开深读的论文，也要保留一个“值得知道但暂不主推”小块，并对每一篇各写一句锐评；每篇严格控制为一句短评，点出为什么值得知道或为什么没进主推，不展开成长分析。这些是同一批 top pool 里的次优项，不是 rejected 列表。",
...
EN_US_PACK = SummaryLanguagePack(
    ...
    task_requirements=(
        ...
        "Within Paper Picks, after the main deep dives, add a small 'Worth Knowing, Not Main Picks' block that gives a one-sentence sharp comment for every remaining paper in today's paper pool that was not expanded as a main pick. Keep each comment to a single compact sentence rather than a mini-analysis. Treat them as secondary reads from the same top pool, not as a rejected-items dump.",
