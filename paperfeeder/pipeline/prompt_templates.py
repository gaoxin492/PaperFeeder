...
ZH_CN_PACK = SummaryLanguagePack(
    ...
    task_requirements=(
        ...
        "论文筛选 section 里，除了主推深读的几篇外，对今天论文池里剩下没有展开深读的论文，也要保留一个“值得知道但暂不主推”小块，并对每一篇各写一句锐评；这些是同一批 top pool 里的次优项，不是 rejected 列表。",
        "今日判断摘要放在最后，用 3 到 5 个短 bullet，集中写今天最重要的判断、工具价值、待确认点，以及最值得读的博客或论文。",
        "博客也要筛选，不是所有博客都值得读。",
        "宁缺毋滥。",
        "具体、可执行。",
        "深度分析要有干货。",
...
EN_US_PACK = SummaryLanguagePack(
    ...
    task_requirements=(
        ...
        "Within Paper Picks, after the main deep dives, add a small 'Worth Knowing, Not Main Picks' block that gives a one-sentence sharp comment for every remaining paper in today's paper pool that was not expanded as a main pick. Treat them as secondary reads from the same top pool, not as a rejected-items dump.",
        "Put Judgment Summary at the end as 3 to 5 short bullets covering the day\'s most important call, practical value, unresolved question, and best non-paper read if relevant.",
